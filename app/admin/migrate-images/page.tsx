  ...
  async function runMigration() {
    ...
    for (const r of restaurants || []) {
      ...
      if (r.logo_url?.includes("deliverlogic")) {
        ...
        if (url) { updates.logo_url = url; success++ } else failed++
        total++
      }
      if (r.hero_image_url?.includes("deliverlogic")) {
        ...
        if (url) { updates.hero_image_url = url; success++ } else failed++
        total++
      }
    ...
    while (true) {
      ...
      for (const item of items) {
        ...
        const url = await migrateImage(supabase, item.image_url, `menu-items/${item.restaurant_id}/${safe}.${ext}`)
        if (url) {
          await supabase.from("menu_items").update({ image_url: url }).eq("id", item.id)
          success++
        } else failed++
        total++
        if (total % 50 === 0) addLog(`Progress: ${total} images`)
        setStats({ success, failed, total })
      }
      offset += 50
    ...
    while (true) {
      ...
      for (const item of items) {
        const ext = item.image_url.split(".").pop()?.split("?")[0] || "jpg"
        const url = await migrateImage(supabase, item.image_url, `catering-menu-items/${item.catering_restaurant_id}/${item.id}.${ext}`)
        if (url) {
        ...
        {(running || done) && (
          <>
            ...
            <div className="bg-slate-800 rounded-xl p-4 h-96 overflow-y-auto font-mono text-sm">
              ...
              {running && <div className="text-amber-400 animate-pulse mt-2">Running...</div>}
            </div>
          </>
        )}
        {done && (
          <div className="mt-6 p-4 bg-green-900 rounded-xl">
            <p className="text-green-300 font-medium">
              Migration complete. Delete app/admin/migrate-images/page.tsx before going live.
            </p>
          </div>
        )}
      </div>
